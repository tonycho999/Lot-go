...
global.firebase = {
    database: () => ({
        ref: () => ({
            update: () => {},
            once: () => Promise.resolve({ val: () => ({ gold: 1000 }) })
        })
    })
};

global.Auth = {
    getCurrentUser: () => ({ uid: 'test_user', email: 'test@lotgo.app' }),
    isAdmin: () => false
};
...
// Manually set gold since we mocked loadUserData
...
// Note: Code maxPrize was updated to 200 in my changes (Mode 0: 200)
// Formula: 200 * 1^2 = 200
